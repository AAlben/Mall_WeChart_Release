            ...
            if (data && data.Item) {
                ...
                if (vItem.SCommdityOptionInfo.length > 0) { //有规格
                    ...
                    $.each(vItem.SCommdityOptionInfo, function (index, item) {
                        var vPrice = (item.Price * vItem.DiscountValue).toFixed(2);

                        vHtml += TxtFormat('<button class="uk-button commodity-type" uId="{0}" uPrice="{1}" onclick="btnOptionClick(\'shop\');" >{2}</button>', [item.ID, vPrice, item.Name])
                    });
